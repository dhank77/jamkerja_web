            ...
            <div className="toolbar mb-5 mb-lg-7 d-flex justify-content-between">
                <div className="page-title d-flex flex-column me-3">
                    ...
                    <ul className="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7 my-1">
                        ...
                        <li className="breadcrumb-item text-gray-600">tugas</li>
                    </ul>
                </div>
                {
                    auth.role.some(ar => ['admin', 'owner'].includes(ar)) &&
                    <div className="d-flex align-items-center py-2 py-md-1" >
                        <Link href={route('pengajuan.tugas.add')} className="btn btn-primary"><b>Tambah</b></Link>
                    </div>
                }
            </div>
                ...
                <div className="card mb-5 mb-xl-8">
                    <div className="card-header border-0 pt-5">
                        <h3 className="card-title align-items-start flex-column">
                            <span className="card-label fw-bolder fs-3 mb-1">Data Pengajuan tugas</span>
                        </h3>
                                ...
                                <tbody>
                                    {data && data.map((u, k) => (
                                        <tr key={k}>
                                            ...
                                            <td>
                                                <Dropdown>
                                                    <Logs model_type="App\Models\Pengajuan\Tugas" model_id={u.id} />
                                                    {
                                                        u.kode_status == 0 && auth.role.some(ar => ['opd'].includes(ar)) ?
                                                            <>
                                                                <Reject routes={route('pengajuan.tugas.reject', u.id)} />
                                                                <Approved routes={route('pengajuan.tugas.approved', u.id)} />
                                                            </>
                                                            : ""
                                                    }
                                                    {
                                                        u.kode_status == 1 && auth.role.some(ar => ['opd'].includes(ar)) ?
                                                            <>
                                                                <Approved text='Selesai' routes={route('pengajuan.tugas.selesai', u.id)} />
                                                            </>
                                                            : ""
                                                    }
                                                </Dropdown>
