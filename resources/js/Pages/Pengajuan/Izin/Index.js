...
export default function Index({ izin }) {

    const { data, meta } = izin
                ...
                <div className="card mb-5 mb-xl-8">
                    <div className="card-header border-0 pt-5">
                        <h3 className="card-title align-items-start flex-column">
                            <span className="card-label fw-bolder fs-3 mb-1">Data Pengajuan izin</span>
                        </h3>
                    </div>
                                ...
                                <tbody>
                                    {data && data.map((u, k) => (
                                        <tr key={k}>
                                            ...
                                            <td>
                                                <div className="text-dark fw-bolder text-hover-primary fs-6">{u.izin}</div>
                                            </td>
                                            <td>
                                                <p>{u.tanggal_mulai}</p>
                                            </td>
                                            ...
                                            <td>
                                                <Dropdown>
                                                    <Logs model_type="App\Models\Pengajuan\PengajuanIzin" model_id={u.id} />
                                                    {
                                                        u.kode_status == 0 ?
                                                            <>
                                                                <Reject routes={route('pengajuan.izin.reject', u.id)} />
                                                                <Approved routes={route('pengajuan.izin.approved', u.id)} />
                                                            </>
                                                            : ""
                                                    }
                                                </Dropdown>
