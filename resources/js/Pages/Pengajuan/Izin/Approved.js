...
export default function Approved({ errors, izin }) {

    const [values, setValues] = useState({
        nomor_surat: izin.nomor_surat,
        tanggal_surat: izin.tanggal_surat,
    ...
    const submit = (e) => {
        e.preventDefault();
        Inertia.post(route('pengajuan.izin.update'), values);
    }
        ...
        <div className="card mb-5 mb-xl-8">
            <div className="card-header border-0 pt-5 flex">
                <h3 className="card-title align-items-start flex-column">
                    <span className="card-label fw-bolder fs-3 mb-1">Form Terima Pengajuan izin</span>
                </h3>
                <div class="card-toolbar">
                    <Link href={route('pengajuan.izin.index')} class="btn btn-dark fw-bolder me-auto px-4 py-3">Kembali</Link>
                </div>
            </div>
